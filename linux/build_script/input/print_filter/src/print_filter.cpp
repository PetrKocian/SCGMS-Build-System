...
HRESULT IfaceCalling CPrint_Filter::QueryInterface(const GUID*  riid, void ** ppvObj) {
	if (Internal_Query_Interface<scgms::IFilter>(print_filter::id, *riid, ppvObj)) return S_OK;
...
HRESULT IfaceCalling CPrint_Filter::Do_Configure(scgms::SFilter_Configuration configuration, refcnt::Swstr_list& error_description) {
	...
	auto example_str = configuration.Read_String(print_filter::rsExample_Config_String);	
	auto example_num = configuration.Read_Double(print_filter::rsExample_Config_Double, 123.0); // default value parameter is optional
...
#if defined(ESP32) || defined (WASM)
	std::wcout << "Event level: " << event.level() << std::endl;
#elif defined(FREERTOS)
	print("Event level: ");
	print_i((int)event.level());
#endif
